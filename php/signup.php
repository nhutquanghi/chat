    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) { // Nếu email là hợp lệ
            ...
            if(mysqli_num_rows($sql) > 0) { // nếu email đã tồn tại
                ...
                if(isset($_FILE['image'])) { // nếu file đã upload
                    $img_name = $_FILE['image']['name']; // nhận 'tên hình ảnh' do người dùng tải lên
                    $tmp_name = $_FILE['image']['tmp_name']; // đây là tên tạm thời được sử dụng để lưu/di chuyển tệp trong thư mục
                    ...
                    $extentions = ['png', 'jpeg', 'jpg']; // đây là một số mở rộng hình ảnh hợp lệ và lưu trữ chúng trong mảng
                    if(in_array($img_ext) === true) { // kiểm tra nếu người dùng đã upload img ext là phù hợp với array extentions
                        ...
                        if(move_uploaded_file($tmp_name, "assets/images/".$new_img_name)) { // nếu user upload img thì move vào folder thành công
